  ...
  if (promo) {
    ...
    if (subtotal >= promo.min_don) {
      ...
      if (typeof showNotification === "function") {
        showNotification(
          `Mã yêu cầu đơn từ ${formatPrice(promo.min_don)}`,
          "warning",
        );
      }
    }
...
function applyPromo() {
  ...
  if (validPromoCodes[code]) {
    ...
    if (subtotal >= promo.min_don) {
      ...
      showNotification(
        `Đơn tối thiểu ${formatPrice(promo.min_don)}`,
        "warning",
      );
    }
  } else {
...
function updatePromoDisplay(info) {
  ...
  if (appliedPromoCode) {
    ...
    display.innerHTML = info
      ? `<div class="auto-promo-info">${info}</div>`
      : "";
  }
}
...
>>>>>>> 1bf398850c8f481c87ac5494a4cce42cf114e382
}
loadCart();
