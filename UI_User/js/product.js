...
function displayProducts(products) {
  ...
  if (!products || products.length === 0) {
    container.innerHTML =
      '<p class="no-products">Hiện chưa có món ăn nào trong danh mục này.</p>';
    return;
  }
    ...
    <div class="product-card">
        <div class="product-image">
            ...
            <div class="product-overlay">
                <button class="quick-view-btn" onclick="openModal(${product.id_mon_an}, '${product.ten_mon}', ${product.gia_ban}, '${product.hinh_anh}')">
...
function addToCartFromModal() {
  ...
  console.log(
    "Thêm vào giỏ từ modal - Product ID:",
    currentProductId,
    "Quantity:",
    quantity,
  );
  addToCartFromProductListing(
    currentProductId,
    currentPrice,
    currentTitle,
    currentImage,
...
const addToCartFromProductListing = (id, price, title, image, quantity) => {
  const product = {
    id: id.toString() + "", // Đảm bảo ID là chuỗi để tránh lỗi khi so sánh
    price: price,
    quantity: quantity,
    title: title,
    image: "../../Image/monan/" + image,
  };
  ...
  if (existingItem) {
    existingItem.quantity =
      parseInt(existingItem.quantity) + parseInt(product.quantity);
  } else {
    cart.push(product);
  }
  ...
  if (false) {
    ...
    if (typeof showNotification === "function") {
      showNotification(
        `Đã thêm ${product.quantity} phần vào giỏ hàng!`,
        "success",
      );
    } else {
